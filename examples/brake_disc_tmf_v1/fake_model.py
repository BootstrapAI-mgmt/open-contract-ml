#!/usr/bin/env python3
"""Reference ``stdio_json`` stub model for ``brake_disc_tmf_v1``.

This is a **fake** model: it implements the full ``stdio_json`` wire format
described below with deterministic, plausible-but-synthetic outputs and **no
actual ML**. Its job is to be the end-to-end fixture — the thing a host
application spawns and an end-to-end smoke test drives — so the wire format
can be exercised end to end before any real trained model is available.
Conflating "the host works" with "the model is good" is exactly what this stub
avoids.
...
* **stderr** — newline-delimited progress events
  (``{"event": "progress", "pct": .., "message": ".."}``) a host may forward
  to its user interface, plus a free-form banner line a host may keep with the
  run record for debugging (not meant for the end user).

**Units.** The host converts display inputs to each field's
``canonical_units`` *before* spawning the model, so this
...
PyInstaller without dragging in any host code.
"""
...
# the result. Overridable to 0 in tests via the environment.
...
def _emit_progress(pct: int, message: str) -> None:
    """Write one progress event to stderr (a host may forward these to its UI)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Free-form banner -> a host may keep it with the run record for debugging.
