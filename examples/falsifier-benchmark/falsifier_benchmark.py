"""
falsifier_benchmark.py - the falsifier benchmark for the surrogate taxonomy
===========================================================================

The small CPU experiment that settles/supports the [OPEN] cells of the
scalar-surrogate selection matrix and grounds the IC-1/IC-2 indifference
tolerances empirically: seeded, CPU-minutes, a printed table with a
...
-------------------------------------------------------------------------
  ...
  B3 GP-Mat52   GP, ConstantKernel*Matern(nu=2.5, ARD) [+White nugget iff
                ...
                design (config label 'mle-sub400') - the GP-at-N=800
                budget risk, remediated by subsampling, never dropped.
...
THE DESIGN-STRUCTURE AXIS. The lattice structure was present from the
first run but at exactly ONE point (14x14=196, noise 0), compared
against LHS-200. One point cannot answer the question the thermal DOE
worked instance raised - whether design structure at fixed N moves the
partition-family inverse and gradient metrics by MORE than N does at fixed
design structure - because it has no N leg. The axis extends the same
...
# machine. Set BEFORE numpy/sklearn import.
...
# round(N**(1/d)) adjustment. Deliberately the SAME grid the
...
# The thermal DOE worked instance's design is a 6x5x4x3x2 full factorial:
...
# constants rather than reading the other's files at run time.
...
def bisection_metrics(model, slices):
    """Adds `detail` (2026-08-21): one (axis, outcome) record per slice.
...
def cv_select_and_fit(fam_key, configs, X, y, range_ref, d, noisy):
    ...
    if fam_key == "B3":
        # Declared budget remediation (GP at N=800 is the budget risk;
        # subsample or declare): N>400 trains on a seeded
...
def cell_list(smoke):
    ...
    # The design axis crossed with dimensionality (2026-08-21). Every lattice
...
def leg_stats(vals):
    ...
    Added 2026-08-21. [V10] printed `mean |dSTRUCT|` over a number its emitter
...
def _cell_hdr(key):
    ...
    # A lattice above d=2 prints its ACTUAL level pattern, so
    # 6x5x4x3x2 is readable in this file without opening the source. The d=2
    # lattice cells that predate 2026-08-21 are left exactly as they were.
    if doe == "lat-aniso":
...
def run(smoke=False):
    ...
    import sklearn
    emit("=" * 78)
    emit("FALSIFIER BENCHMARK - scalar-surrogate taxonomy")
    emit("=" * 78)
    ...
    # The two legs the thermal DOE worked instance asked for: does design STRUCTURE at
  ...
  points, sweeps, and rays (seed=0).""")
    ...
    # V10 design-structure axis. V8 above keeps the original
    ...
    if leg_tab:
        ...
        v10 = ["B-axis DOE-geometry cell crossed with N, bearing on R1 (the",
               ...
               "the thermal DOE worked instance, whose 6-level full factorial reproduced",
               "the taxonomy's DIRECTION on every axis and missed its MAGNITUDE",
        ...
        v10 += [
            ...
            "  families sit at %.3f on the %dx%d lattice, inside the taxonomy's band."
            ...
            "of its supporting evidence, not its direction. The taxonomy's scope admits both",
            ...
            "V1's denominators differ from the first-run file for that reason. No",
            "IC-1 definition changed; the cell set grew. Queued as follow-up work."]
        vd("V10 design-structure", v10)

    # ======================================================================
    # 2026-08-21. [V10-A1] annotates [V10] WITHOUT editing it;
    # [V11] reads out d=5 cells this file has carried since its first run and
    ...
    if leg_tab and part9e:
        ...
        a1 = ["Annotation BESIDE [V10], not an edit to it. [V10] is a dated",
              "measurement record; the convention (2026-08-17) is that what a",
              "block found is never rewritten, only annotated with what was",
              "found out about it later. Two",
        ...
        vd("V10-A1 2026-08-21 annotation", a1)
    ...
    if part9e and k_d2 in results and k_d5b in results:
        ...
        v11 = ["READ-OUT, not a new measurement (2026-08-21). Every number in this",
               "block comes from cells this file has carried since its first run.",
        ...
        v11 += ["dDIM = d=5 N=%d minus d=2 N=%d = %+.3f  (%.1f slices)"
                ...
                "effect, and it has been in this file since its first run.",
                "",
    ...
    if part9e and len(new9e) == 4:
        ...
        v12 = ["The design axis crossed with DIMENSIONALITY, and level ANISOTROPY",
               "made expressible for the first time (2026-08-21). Before this run",
               "every lattice cell in this file was d=2, so [V10]'s design axis had",
...
def main():
    ap = argparse.ArgumentParser(description="falsifier benchmark for the surrogate taxonomy")
