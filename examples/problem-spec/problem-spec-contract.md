# Problem-spec contract — TASK-9 surrogate selector

The machine-readable definition of exactly what `select_surrogate.py --spec`
expects. This is the requirements-selection analogue of the data-contract
sheets the TASK-8 examples ship (`EXAMPLES-ARC-SCOPING.md` §5.4): the same
role, but the thing being contracted is a description of *requirements*
rather than a table of CAE runs — the selector never sees your data.

Fill in `problem-spec-template.yaml` against this sheet, or answer the same
questions conversationally with `--interactive` and let the tool write the
file for you.
...
- `d_bin` is recorded, not branched on: every bin is inside the taxonomy's
...
- **`needs_equation_interpretability` is about the artifact, not about
  ...
  requirement; that is what §7 of `WALKTHROUGH.md` is for.
...
useful finding about the problem — roughly 30% of the taxonomy's full
scenario enumeration lands there.

---

## 4. Worked examples in this folder

| file | what it is | expected verdict |
|---|---|---|
| `spec-HT-thermal-doe.yaml` | the 720-row full-factorial thermal DOE from the task card's provenance — inverse solve, sensitivities, smooth sweeps, Excel-tier deployment | **B1**, admissible {B1, B2, B4, B3} |
| `spec-SELF-selector.yaml` | the selector's own problem: predict a family from a problem's shape | **B6**, admissible {B6} |
| `spec-nvh-rig-peak-spl.yaml` | measured NVH rig peak sound-pressure level: kinked, noisy, ~1,200 runs, no smoothness requirement | **B6**, admissible {B6, B7, B10} |
...
*Contract authored SESSION-9B-2 (2026-08-15). The field schema, vocabularies
and coherence rule are read directly from `surrogate_selector.py` at run
time; this sheet documents them and does not redefine them. If the two ever
