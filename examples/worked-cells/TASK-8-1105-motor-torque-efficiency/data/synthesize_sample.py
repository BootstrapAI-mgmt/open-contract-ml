"""
TASK-8-1105 Motor Torque & Efficiency Surrogate — SAMPLE-DATA SYNTHESIZER
========================================================================
...
The two load-bearing physical mechanisms the A.5.1 entry names as the
dominant surrogate-failure modes are baked into the response surface so
the example is honest about them:

  * Magnetic saturation (Pitfall (i), the DOMINANT low-frequency-EM
  ...
  * Permanent-magnet temperature dependence (Pitfall (ii)): B_r(T)
...
See data-contract.md and WALKTHROUGH.md §4 for the illustrative-numerics
framing.
...
def _sample_design(rng):
    ...
    its operating point, drawn from the TASK-1 A.5.1 Inputs field:
