"""
TASK-8-1106 Injection-Molding Warpage Surrogate — SAMPLE-DATA SYNTHESIZER
========================================================================
...
The three sub-physics that A.10.2 names (Kennedy2013 / TASK-1 §6 A.10.2)
— non-Newtonian fill, non-isothermal cool, and (for fibre-reinforced
grades) fibre-orientation evolution — all feed these three headline
outputs. The two A.10.2-named dominant *deployed-surrogate* failure
modes are baked into the response surface so the example is honest:

  * Process-parameter-envelope extrapolation (Pitfall (iv), the dominant
  ...
  * Polymer/fibre-class sensitivity (Pitfall (ii)): glass-fibre content
...
def _sample_design(rng):
    ...
    the part geometry, drawn from the TASK-1 A.10.2 deployed-tabular
    Inputs field (process parameters + part-design parameters):
      ...
      - 1 material: glass-fibre content (the fibre-reinforced-thermoplastic
                    warpage driver A.10.2 puts on the critical path)
