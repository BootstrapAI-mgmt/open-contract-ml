# TASK-8-1201 -- Predicting the Temperature Field of a Cooled Component
...
> (`EXAMPLES-ARC-SCOPING.md`). This is example **`TASK-8-1201`**: the
> ID encodes its slide-9 grid cell -- `n=1` (parametric-input row),
> `m=2` (regular-grid-field-output column), example `01`. It is the
> **first example outside Bucket B1** in the arc and the first to
> predict a whole **field** rather than a scalar. It bridges directly
> from `TASK-8-1104` (peak temperature): the inputs are the same seven
> thermal parameters; only the output grows, from the single peak
...
The scalar example `TASK-8-1104` built a surrogate for the *peak*
...
parameters as `TASK-8-1104`) and returns a *field on a regular grid* (a
temperature at every voxel of a fixed 12 x 8 x 8 lattice -- 768 numbers).
On the BRIDGE-A slide-9 grid that is one cell to the **right** of the
top-left corner -- *parametric input -> regular-grid-field output* (cell
`n=1, m=2`) -- which is **Bucket B2, the regular-grid field surrogate**.

> **Why this example is allowed to step outside B1 (the admission
> record).** The arc deliberately built its first six examples in the
> simplest grid cell (B1, scalar output) before stepping anywhere else.
> This example is the first admitted step. It clears the arc's
...
>    scalar field T(x) on regular voxel grids ... -> 3-D U-Net / dense
>    3-D CNN for regular-grid thermal fields." This example is exactly
>    that row's worked instance.
> 3. **It is a single-step bridge from a built example:** it changes
>    *one* thing relative to `TASK-8-1104` -- the output goes from a
...
inventory record: `EXAMPLES-ARC-SCOPING.md` section 11.2 (the rightward-
cell lead candidate) and section 4.
...
- **At least 80 runs**; 200 - 400 is healthy. The field output does not
  ...
  the same seven-parameter design space as `TASK-8-1104`, so the minimum
...
- The seven inputs are listed in `data/data-contract.md`. They are
  identical to `TASK-8-1104`: length along the cooling axis,
...
- **Single-class boundary conditions per dataset.** Train one surrogate
  ...
  -- this is TASK-1 A.3.1 Pitfall (ii).
...
> scalar `TASK-8-1104` used, evaluated at every voxel of the grid). You
...
The input columns, in order, are the same seven as `TASK-8-1104`:
...
- **Peak-temperature mean absolute error (deg C)** -- how far off the
  ...
  the field surrogate back to the scalar `TASK-8-1104` question: if you
...
scalar thermal example (`TASK-8-1104`) builds on; TASK-1 A.3.1 anchors it
in the standard heat-transfer references ([Bergman2017] for the
conduction equation and the Biot / Fourier dimensionless framework,
[Patankar1980] for the discretisation family). A surrogate trained on
...
-- the same closed form the scalar `TASK-8-1104` used for its peak, now
...
keeps the arc's click-to-run promise. The convolutional / U-Net variant is
the **planned-but-deferred upgrade** -- the field-example analogue of the
scalar examples' deferred Gaussian-Process variant -- and is recorded in
`EXAMPLES-ARC-SCOPING.md` section 11. The bucket is **B2 either way**: the
classifier attribute is the regular-grid field *output*, not the decoder's
internals.

The Biot number `Bi = h * L_c / k` and the Fourier number `Fo` are the
load-bearing dimensionless groups for the heat equation; per ROADMAP A.3,
non-dimensionalising the problem in Biot / Fourier scaling is the single
load-bearing surrogate-generalisation lever for thermal entries. This
example ships the *dimensional* form (raw mm / W / deg C inputs) for
reader-clarity; a future iteration may add a non-dimensional variant.
...
- **`TASK-8-1104` -- Peak Temperature of a Cooled Component.** The scalar
  example this one bridges from -- the *same* seven thermal inputs, with
  the output a single peak number instead of the whole field. Reading the
  two back to back is the cleanest way to see what "stepping one cell
  right on the slide-9 grid" means: same physics, same inputs, the output
  grows from a scalar (Bucket B1) to a field (Bucket B2).
- **`TASK-8-1101` -- Fatigue Life of a Loaded Bracket.** The arc's first
  ...
  path; the template proof the whole arc is built on.
- **`TASK-8-1102` -- Drag and Lift of an Off-Road Vehicle.** The arc's
  external-aero B1. Its downward neighbour on the grid (`n=2, m=1`,
  drag-from-a-shape-image) is the *other* single-step move off B1 -- a
  richer input rather than a richer output -- and is the recommended
  second non-B1 build after this one.
- **The BRIDGE-A deck** in `presentations/` for the conceptual framing of
  ...
  shape. This example is the worked instance of the grid's `n=1, m=2`
  cell.
...
install step: download the `TASK-8-1201-temperature-field/` folder,
double-click `Train-Temperature-Field.exe`, then double-click the
`Predict-Temperature-Field.exe` the trainer drops into a `trained-model`
