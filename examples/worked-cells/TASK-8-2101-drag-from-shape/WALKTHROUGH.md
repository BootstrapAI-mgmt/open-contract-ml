# TASK-8-2101 -- Predicting Drag from a Shape Image

### A step-by-step worked example for CAE engineers -- no ML background needed

> Part of the CAE-ML step-by-step examples arc (`EXAMPLES-ARC-SCOPING.md`).
> This is example **`TASK-8-2101`**: the ID encodes its slide-9 grid cell
> -- `n=2` (regular-grid / image-like input row), `m=1` (scalar-output
> column), example `01`. It is the **second example outside Bucket B1** in
> the arc and the first to take an **image** as input. It bridges directly
> from `TASK-8-1102` (drag and lift): the target is the same drag
...
The parametric drag-lift example (`TASK-8-1102`) built a surrogate that
...
(CD, and CL). On the BRIDGE-A slide-9 grid that is one cell **down** from
the top-left corner -- *image-like input -> scalar QoI output* (cell
`n=2, m=1`) -- which is **Bucket B2** routed by the *input* shape.

> **Why this example is allowed to step outside B1 (the admission
> record).** This is the second admitted step off the B1 cell (the field
> example `TASK-8-1201` was the first). It clears the arc's admission
> checklist (`EXAMPLES-ARC-SCOPING.md` section 7, the "S-4" gate) on three
> points:
> 1. **Its grid cell and bucket are named explicitly:** slide-9 cell
>    `n=2, m=1`; Bucket **B2**, routed **by input shape**. B2's defining
>    test is written about grid *outputs*, but this candidate has a grid
>    *input* and a scalar output; it routes to B2 because the
>    convolutional / image inductive bias is the load-bearing classifier
>    attribute. This is the same by-shape edge-case routing TASK-4 section
>    6 resolves, disclosed here rather than smoothed over.
> 2. **It cites the canonical catalog row it instantiates:** TASK-1 A.1.1
>    names the geometry-as-image encoding ("geometry encoded as an SDF,
>    surface-normal map, or unwrapped surface texture") and the
>    image-input aero CNN (the encoder half of the surface-pressure U-Net,
>    used here for regression).
> 3. **It is a single-step bridge from a built example:** it changes *one*
>    thing relative to `TASK-8-1102` -- the input goes from a parameter
>    vector to a shape image -- on a problem the reader has already seen as
>    a B1. The physics and the CD / CL target are unchanged.

Catalog reference: analysis **A.1.1** (CFD, External-Aero) in
`tasks/TASK-1-cae-analysis-catalog.md`. Arc inventory record:
`EXAMPLES-ARC-SCOPING.md` section 11.3 (the downward-cell lead candidate)
and section 4.
...
- **One operating point per dataset.** This example fixes the operating
  ...
  `TASK-8-1102` (which takes the operating point as input).
...
thing for a model to learn from. TASK-1 A.1.1 names the SDF as one of the
standard geometry-as-image encodings for exactly this reason.
...
3. **One operating point only.** This surrogate fixes the speed and yaw and
   ...
   `TASK-8-1102` (which takes the operating point as input) is for, or a
...
**Why a dense regressor and not a convolutional network.** TASK-1 A.1.1
and TASK-4's Bucket-B2 roster name the canonical *production* architecture
for image-input aero as a **2-D CNN** -- the encoder half of the
surface-pressure U-Net, with shared convolutional filters that exploit the
image's spatial structure and translation-invariance, topped with a
global-pooling head that outputs the scalar coefficients. That architecture
typically reaches a tighter accuracy and scales to larger, higher-resolution
images, but it is GPU-friendly and materially heavier to train and to
package as a double-click executable. This example ships the simpler dense
regressor so it trains on a CPU in well under a minute and keeps the arc's
click-to-run promise. The convolutional CNN is the **planned-but-deferred
upgrade** -- the image-example analogue of the field example
`TASK-8-1201`'s deferred 3-D U-Net, and of the tabular examples' deferred
Gaussian-Process variant -- and is recorded in
`EXAMPLES-ARC-SCOPING.md` section 11. The bucket is **B2 either way**: the
classifier attribute is the regular-grid image input, not the regressor's
internals.
...
- **`TASK-8-1102` -- Drag and Lift of an Off-Road Vehicle.** The parametric
  example this one bridges from -- the *same* external-aero physics and the
  *same* CD / CL target, with the input a row of assembly / operating
  numbers instead of a shape image. Reading the two back to back is the
  cleanest way to see what "stepping one cell down on the slide-9 grid"
  means: same physics, same target, the input grows from a parameter
  vector (Bucket B1) to an image (Bucket B2) -- and it shows *why* a shape
  image can carry information a parameter vector could not.
- **`TASK-8-1201` -- Temperature Field of a Cooled Component.** The *other*
  single-step move off the B1 cell -- a richer *output* (a field) rather
  than a richer *input* (an image). The two together show the two
  genuinely-distinct shape steps the arc takes from the B1 corner:
  richer-output (rightward, `n=1,m=2`) and richer-input (downward,
  `n=2,m=1`).
- **`TASK-8-1101` -- Fatigue Life of a Loaded Bracket.** The arc's first B1
  and the template proof the whole arc is built on.
- **The BRIDGE-A deck** in `presentations/` for the conceptual framing of
  ...
  shape. This example is the worked instance of the grid's `n=2, m=1`
  cell.
...
This example runs as standalone executables on Windows. There is no install
step: download the `TASK-8-2101-drag-from-shape/` folder, double-click
`Train-Drag-Shape.exe`, then double-click the `Predict-Drag-Shape.exe` the
trainer drops into a `trained-model` folder. No Python required.

(If you are running the *interim* form -- `Train.bat` / `Predict.bat` in
`trainer/` -- Python 3.10+ with `scikit-learn` and `numpy` installed is
required. The interim form ships in the source repo as a fallback for build
hosts that do not yet have the standalone build; the reader-facing form is
