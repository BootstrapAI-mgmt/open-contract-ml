# TASK-9 H-T Worked Instance — Data Contract

> This is the exact specification of the dataset `run_ht_instance.py`
> expects. Unlike the TASK-8 examples, this folder is not asking you to
> supply your own CAE results — the whole point of the instance is that
> everyone runs the *same* 720 rows, so that the numbers in taxonomy
> §8 can be reproduced rather than believed. `synthesize_sample.py`
> regenerates the file byte-for-byte on demand.
>
> All numeric values are *illustrative*. The functional form is a
> two-node steady-state energy balance, not a substitute for a
> conjugate thermal-FE solve. It exists so that the taxonomy's
> requirements argument can be watched happening on a surface whose
...
it is the sentence the taxonomy card was written around:
...
cannot, is the entire subject of TASK-9.
...
1. **The design is a full factorial, and that matters.** Most CAE
   ...
   a grid, and because the debrief's third bullet is specifically about
   grids: *"staircase interpolation between full-factorial levels
   (engineers query between nodes)."* A partition-based model fitted to
   a 6-level grid has **five** places it can put a split on
...
2. **Six clearance levels is few, on purpose.** It is enough to see the
   ...
   by recall. Widening the grid would soften the effect; the DOE the
   card describes had this shape and the instance keeps it.
3. **The three heat-path targets sum to `q_W` by construction.**
   ...
   deliberately **not** one of the head-to-head's metrics — adding it
   would be scope the brief did not ask for. It is noted so the next
   session does not mistake the redundancy for a mistake.
4. **Deterministic, noise-free, no seed.** There is no measurement
   ...
   bisection to 10⁻¹⁰ K. This matches the selector spec for the case
   (`spec-HT-thermal-doe.yaml`: `deterministic: yes`, `noise: none`) —
   if you change one, change the other, or the selector will be
   answering about a different problem than the one in this folder.
   Project memory records why the response surface must be a
   deterministic function of the inputs: a per-row `rng.uniform()`
   inside the response evaluation becomes irreducible noise and destroys
   the fit (TASK-8-1104 first attempt, avg R² 0.025 → 0.890).
...
Taxonomy §8 describes this instance as carrying *"physics-engineered
features (ΔT terms and a T⁴ radiation term)"*. Those features are **not**
in this CSV — they are built from these columns at fit time, in
`run_ht_instance.py`'s `make_features`, and handed identically to all
three families:
...
them into accuracy. That is what the taxonomy means by partitioning
buckets by estimation principle rather than by function class
(doc §2.1/§2.2).

## How this maps to the taxonomy

This is the instance taxonomy §8 walks rule-by-rule, and the selector
ships it as a filled sample spec at
`examples/TASK-9-surrogate-selector/data/spec-HT-thermal-doe.yaml`.
Running that spec through the selector gives top-1 **B1**, admissible
**{B1, B2, B4, B3}**, with **B6 / B8 / B10 excluded by R1** — the rule
that reads `needs_inverse`, `needs_gradients` and `needs_smooth_sweeps`.
...
Catalog anchors: TASK-1 §6 A.3.1 (FEA Thermal Steady); TASK-3 §6 A.3.1 →
Bucket B1; TASK-4 §5 B1 roster row. Textbook anchor for the conduction
and radiation framing and for the Ra ≈ 1708 enclosure criterion:
[Bergman2017].

**Companion material.** `examples/TASK-9-surrogate-selector/` is the tool
that reaches this verdict from a filled spec; this folder is the
evidence that the verdict is earned. The contrast case worth reading
next to it is the third sample spec in that folder — a measured NVH rig,
kinked and noisy, with no smoothness requirement — where the same
taxonomy returns **B6**. The pair is the argument that the taxonomy
prefers whichever families survive the declared requirements, not
whichever families are smooth.
