# Injection-molding warpage example — Data Contract
...
1. **Eight inputs, three targets.** Columns 1–8 are the parameters you
   ...
   polymer grade — see Rule 5) through. Like `motor-torque-efficiency`
   this is a **three-value output bundle** example; the recipe is
   identical to the structural / thermal / motor examples and simply
   carries three targets. The analysis is injection-molding process
   simulation; [Kennedy2013] is the foundational textbook anchor for the
...
2. **One row per simulation run.** Each row is one completed
   ...
   tabular-surrogate target flavor (the deployed industrial pattern at
   injection-molded-component design organisations regresses
   process-and-design parameters → headline as-molded-quality
...
3. **Targets are linear-scale, not logarithmic.** Warpage, spring-back,
   ...
   `log10(1.8)`. (A target that spans many decades, such as a fatigue
   life, is the case where a log transform is warranted.)
...
5. **Single polymer-class per dataset (the load-bearing discipline).**
   ...
   (e.g. one PP-GF family, or one PA6-GF family — not a mix). A
   surrogate trained on one polymer class cannot be silently used on a
...
6. **One flow-formulation class per dataset.** A surrogate trained
   on Hele-Shaw / 2.5-D fill data
...
9. **Spread your runs across the ranges, and cover the process-window
   ...
   the full process window, not just the nominal recipe (the dominant
   *deployed-pattern* surrogate-failure mode: a surrogate trained on
   one narrow process-parameter envelope mispredicts qualitatively
   outside it). A surrogate that only sees the
...
- **`peak_residual_stress_mpa`** is the peak frozen-in residual stress in
  ...
  downstream structural analysis of the part; the
  architecturally-correct treatment chains this manufacturing output
  into the downstream structural model rather than discarding it.
...
is the B1 example that lives in the manufacturing-process
(injection-molding) domain.

Anchors:

- Injection-molding process simulation: governing equations and
  fidelity tiers. Textbook anchor: [Kennedy2013]
  ...
  strength scalars) is the deployed industrial pattern; GBM / MLP are
  the usual tabular architectures ([Chen2016] / [Ke2017] /
  [Goodfellow2016]). The fibre-orientation-tensor *field* prediction
  flavor is a different bucket and a different architecture (3-D CNN /
  GNN — research-active, not this tabular surrogate).
- Headline as-molded-quality scalar regression is the deployed,
  production-mature surrogate target; full-field
  fibre-orientation-tensor regression is the research-active flavor in
  a different bucket.

**Companion examples.** `motor-torque-efficiency` is the closest sibling
architecturally — the other **three-target** multi-output
gradient-boosted-tree example, same recipe; this example applies that
identical three-target pattern in the manufacturing-process domain.
`peak-temperature` and `peak-stress` are the two-target siblings.
Reading the manufacturing example after the motor example shows the
same three-target B1 pattern transferring across yet another physics
family (low-frequency EM → injection-molding process).
