# Predicting the Temperature Field of a Cooled Plate
...
It is the **second field-output** worked cell, and it is deliberately
the *same* kind of model as `temperature-field` (the cooling-fin
temperature field) on a **different component geometry**. If you have
read `temperature-field`, you will meet **no new machine-learning idea
here** -- only a different physical setup wearing the same data
contract. That is the point of the pairing: it shows the field-output
recipe is not tied to one component shape.
...
`temperature-field` built a field surrogate for an extruded **cooling fin** --
...
cooling), different geometry and boundary conditions.
...
lattice -- 1024 numbers). On the input/output grid that is one cell to
the **right** of the top-left corner -- *parametric input ->
regular-grid-field output* (cell `n=1, m=2`) -- which is **Bucket B2,
the regular-grid field surrogate**. It is the second worked cell in this
grid cell, after `temperature-field`.

> **Why a second example in the same cell.** The worked cells start in
> the simplest grid cell (B1, scalar output), then step one cell right
> (`temperature-field`) and one cell down (`drag-from-shape`). This
> example is a **second instance of the rightward (n=1, m=2) cell**
> rather than a step to a new cell:
...
> 2. **It is a canonical B2 case:** a volumetric temperature field on a
>    regular voxel grid, whose usual production architecture is a 3-D
>    U-Net or a dense 3-D CNN. A flat heat-spreader with a discrete
>    source is a textbook electronics-cooling instance of it.
> 3. **It is the cheapest kind of new example: a second geometry family
>    in a built cell.** It introduces **no new ML idea** relative to
>    `temperature-field` -- same Bucket B2, same dense field-decoder, same
>    data-contract shape -- and changes only the *geometry /
>    boundary-condition family* (a plate with an in-plane-spreading
>    source instead of a fin) and the *grid shape* (16 x 16 x 4 instead
>    of 12 x 8 x 8). Its pedagogical job is to show the field-output
>    template generalises across geometry families within a domain, not
>    only across domains.
...
- **At least 80 runs**; 400 - 600 is healthy, and a field with in-plane
  ...
  decoder benefits from more examples than `temperature-field` needed. A
...
- **Single-class boundary conditions per dataset.** Train one surrogate
  ...
  unobserved hidden variable that silently degrades accuracy
...
idea this example asks you to absorb (the same idea as `temperature-field`):
...
Note the grid shape (16 x 16 x 4) is **different** from `temperature-field`'s
...
is a touch higher than `temperature-field`'s ~0.08 because the plate's in-plane
...
thermal example (`peak-temperature`) and the fin field example
(`temperature-field`) build on, anchored in the standard heat-transfer
...
same architecture as `temperature-field`** -- only the input count (8 vs
7), the grid shape (16 x 16 x 4 vs 12 x 8 x 8), and the training data
differ. That is the whole point of a second-instance example: the recipe
is unchanged.
...
cleanly. Where `temperature-field`'s fin field was governed by conduction along
...
dense decoder so it trains on a CPU in well under a minute -- the same
choice `temperature-field` made. The convolutional / U-Net variant is
the natural upgrade -- the field-example analogue of the scalar
examples' Gaussian-Process variant. The bucket is **B2 either way**: the
classifier attribute is the regular-grid field *output*, not the
decoder's internals.

The Biot number `Bi = h * L_c / k` is the load-bearing dimensionless group
for the heat equation, and non-dimensionalising the problem in Biot /
Fourier scaling is the single load-bearing surrogate-generalisation lever
for thermal problems. This example ships the *dimensional* form (raw mm /
W / deg C inputs) for reader-clarity; a non-dimensional variant is a
natural extension.
...
- **`temperature-field` -- Temperature Field of a Cooled Component
  (fin).** The closest sibling -- the *same* Bucket-B2 field-output
  recipe on a *different* geometry (an extruded cooling fin instead of a
  flat plate) and a different grid (12 x 8 x 8 instead of 16 x 16 x 4).
  Reading the two back to back is the cleanest way to see that the
  field-output template generalises across geometry families, not just
  across physics domains -- which is exactly why this example exists.
- **`peak-temperature` -- Peak Temperature of a Cooled Component.** The
  scalar (Bucket B1) thermal ancestor of the field line: the same
  physics family, output a single peak number instead of a whole field.
  The B1 -> B2 step (scalar -> field) is what `temperature-field` and
  this example both build on.
- **`drag-from-shape` -- Drag of a Body from its Shape Image.** The
...
`Train-Plate-Temperature-Field.exe`, then double-click the
