"""
Plate Temperature-Field Surrogate example -- SAMPLE-DATA SYNTHESIZER
=======================================================================
...
This is the SECOND field-output worked cell (grid cell n=1, m=2,
bucket B2) and the second instance of that cell in the SAME thermal
domain (steady-state thermal) as the temperature-field example -- but on
a DIFFERENT component geometry and boundary-condition family. Where the
temperature-field example modelled an extruded
...
different; only the domain (steady thermal) and the B2 field-output
...
DIFFERENT from the temperature-field example's (12,8,8) -- a deliberate
demonstration that
...
def _sample_design(rng):
    ...
    Eight inputs. The first six mirror the thermal vocabulary of the
    peak-temperature example (geometry + material + convection + ambient);
