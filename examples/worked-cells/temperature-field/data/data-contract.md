# Temperature-field example -- Data Contract
...
This is the first **field-output** worked cell. The tabular examples
predict one or a few scalar numbers per run. This one predicts a whole
**temperature field**: the temperature at every voxel of a fixed regular
grid, laid out across the component. The input side is unchanged from
the scalar peak-temperature example (`peak-temperature`) -- the same
seven thermal parameters. Only the output grows from a number to a
field.
...
1. **Seven inputs, 768 field targets.** Columns 1-7 are the parameters
   ...
   operating-point + 1 material -- the same vocabulary as
   `peak-temperature`). Columns 8-775 are the temperature field your
   steady-state run produced, resampled onto the fixed grid. Column 776
...
6. **Single-class boundary conditions per dataset.** The contract assumes
   ...
   boundaries, train a separate surrogate per class (boundary-condition
   class inheritance).
...
9. **At least 80 rows; more is better.** A field surrogate has many more
   ...
   space is the same seven-parameter space as `peak-temperature`, so the
   minimum row count is the same (80). 200-400 well-spread runs is a
   healthy
...
This example sits one cell to the **right** of the top-left B1 cell on
the input/output grid: *scalar / vector (parametric) input -> field on a
regular grid output* (cell `n=1, m=2`). That cell is **Bucket B2 -- the
regular-grid field surrogate**. It is the first worked cell outside B1
and the first to live in the field-output column.
...
- A volumetric temperature field on a regular voxel grid (electronics
  ...
  is the canonical B2 case; its usual production architecture is a 3-D
  U-Net or a dense 3-D CNN. The B2 defining test -- the output is a
  field on a regular Cartesian grid and its spatial structure is
  exploitable -- fires.
- The scalar B1 version of the same analysis is the
  parameter-to-peak-temperature regression (`peak-temperature`); this
  example is its field variant.
...
the seven inputs to the 768-voxel field) so the example trains on a CPU
in seconds-to-minutes. The U-Net / CNN with spatial convolutions is the
heavier production variant; it is named in WALKTHROUGH.md section 7 as
the upgrade, exactly as the scalar examples name their GP variant. The
bucket is B2 either way -- the classifier attribute is the regular-grid
field *output*, not the specific decoder internals.

**Companion example.** `peak-temperature` is the closest sibling: the
same seven thermal inputs, with the output stepped from the single peak
number to the whole field. Reading the two together is the cleanest way
to see what "stepping one cell right on the grid" means in practice.
