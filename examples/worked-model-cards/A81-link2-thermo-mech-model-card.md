...
## Link 2 — A.9.2 Thermo-Mechanical · Bucket B7 (Latent-Space Coupled-Dynamics ROM)
### Model Card v0.1

> Companion artifact to `WORKED-INSTANCE-A81-link2-thermo-mech-contract.md`.
> The contract sheet covers what the model produces and consumes; this
> model card covers what the model *is* — encoder/decoder/Neural-ODE
> topology, hyperparameters, training configuration with adjoint
> backprop, full loss formula, UQ approach, and alternatives considered.
>
> **Disclosure** inherits from the contract sheet §8 in full. Specific
> hyperparameter values, latent dimension, GNN depth/width, Neural-ODE
> tolerances, and λ-weights below are *illustrative configurations*; the
> autoencoder + Neural-ODE family choice is anchored to Pattern 6 Layer 3.
...
|---|---|
| Chain link | 2 of 3 |
| CAE analysis | A.9.2 — Multiphysics: Thermo-Mechanical (`tasks/TASK-1-cae-analysis-catalog.md` §6) |
...
| Inference latency | 10–100 ms per soak-window evaluation (deck slide 13); CPU inference ~500 ms – 2 s |
...
where
  ...
  L_sym    = symmetric-tensor preservation penalty: penalises any predicted
                ...
                eps_p[j,i]) deviates from zero. Per ROADMAP A.2 pitfall
                (predict 6 independent components, not 9).
...
**Asymmetry between training and inference loss application.** L_recon and L_dyn are summed over training snapshots; at inference, a single peak-cyclic field is returned rather than a snapshot trajectory (per contract §3.4). The reconciliation between snapshot-level supervision and peak-level deployment is one of the format deviations flagged in contract §10.

The chain-composite `L_endpoint` term is added during chain-level fine-tuning per the chain-level contract sheet.
...
| Output to chain | Full-tensor `epistemic_sigma_eps_p` + `latent_disagreement` scalar per contract sheet §4.2 |
...
|---|---|---|
| Autoencoder + Neural-ODE family choice | **Anchored** | Pattern 6 Layer 3; Chen2018 (Neural-ODE) |
| GNN encoder/decoder for unstructured mesh | **Anchored** | Battaglia2018 graph-network framework; deck slide 9 |
| Symmetric-tensor preservation in loss | **Anchored** | ROADMAP A.2 pitfall |
| Coupling-class enforcement protocol | **Anchored** | A.9.x cluster framing; [Bazilevs2013] |
