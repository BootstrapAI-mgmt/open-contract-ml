...
## Link 3 — A.8.1 Fatigue Post-Process · Bucket B1 (Tabular, GBM + GP)
### Model Card v0.1

> Companion artifact to `WORKED-INSTANCE-A81-link3-fatigue-contract.md`.
> The contract sheet covers what the model produces and consumes; this
> model card covers what the model *is* — GBM ensemble + GP companion,
> hyperparameters, training configuration, full loss specification, UQ
> decomposition, and alternatives considered.
>
...
> hyperparameter values and kernel choices below are *illustrative
> configurations*; the GBM + GP family choice is anchored to TASK-3 §6
> A.8.1 + TASK-4 §5 B1 roster.
...
| Production-deployment maturity | Strongest in catalog (deck slide 19) | Less common at OEM scale | GBM is the design-iteration deliverable; GP is the certification companion |
...
This pair structure is anchored to TASK-3 §6 A.8.1 mapping rows (GBM as production-default, GP as the calibrated-UQ companion) and TASK-4 §5 B1 roster (both members of B1).
...
| Element | Value |
|---|---|
| Training corpus | 806 rows (per dataset card §6 splits) |
...
| Total compute | < 10 minutes on a single CPU node, all 5 ensembles together (deck slide 13 — "GBM trains on CPU in minutes for typical 10³ – 10⁴ row corpora") |
...
The chain-composite `L_endpoint` term — special-case treatment because GBM does not gradient-fine-tune cleanly — is addressed in the chain-level contract sheet. Likely treatment: freeze link-3 weights during chain-level joint fine-tune; back-propagate `L_endpoint` only through links 1 and 2.
...
| MLP regressor | Less inductive-bias-fit for tabular data than GBM; rejected per TASK-4 §3 B1 framing — GBM/GP/MLP are alternative B1 family members but GBM is the production-default per TASK-3 §6 A.8.1 |
...
| Pure rainflow + Miner post-processor (no ML) | The thing we replace; not an ML alternative. Remains the *ground-truth* via the dataset card §3 solver provenance |
...
| **Deployment maturity (this link)** | **DEMONSTRATED** | Deck slide 18 + TASK-3 §6 A.8.1 industry-context paragraph + deployment-evidence sub-section |
