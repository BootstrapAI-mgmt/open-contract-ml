"""Shared helpers for the corpus consumer: schema, loading, stats, I/O.

The corpus schema: one row per (case_id, component_name); swept parameters
as dotted-path columns; ``mesh_size_mm`` / ``n_cells`` inputs;
``T_max_K`` / ``T_min_K`` / ``T_mean_K`` / ``pass_fail_status`` / ``margin_K``
targets; ``wall_time_s`` / ``cost_usd`` / ``convergence_status`` /
``final_residual_T`` metadata. Extra columns (a structural sweep's strength
...
CORPUS_TARGETS = ("T_max_K", "T_min_K", "T_mean_K", "pass_fail_status", "margin_K")
...
REQUIRED_COLUMNS = IDENTIFIER_COLUMNS + MESH_COLUMNS + CORPUS_TARGETS + META_COLUMNS
