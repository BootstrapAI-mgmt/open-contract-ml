...
Implements section 4.2 V1, V2 and V3 of VERTICALS-BUILD-TEST-VALIDATE-PLAN.md
for artifacts produced by train_b1.py:
...
Usage:
  python model_gate.py --corpus <dir>/training_corpus.parquet --gate-report out/corpus_gate_report.json
                       --rules rules/pv-c2.physics_rules.json --model out/model --card out/model_card.json
...
def run_gate(corpus_path: Path, gate_report: Dict[str, Any], rules: Dict[str, Any], model_dir: Path) -> Dict[str, Any]:
    ...
    card = {
        ...
        "threshold_provenance": "PROVISIONAL defaults per VERTICALS-BUILD-TEST-VALIDATE-PLAN.md section 3.2 / section 9 item 9",
    }
    return card
