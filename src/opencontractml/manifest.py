"""Manifest validation: the package contract as code.
...
gives downstream code (a registry, a runner) a typed, validated view of one.

Validation is layered, and the order is load-bearing:

1. **Structure** — the manifest is validated against the canonical
   ``schemas/package-v1/manifest.schema.json`` (JSON Schema Draft 2020-12). The schema is
...
3. **Semantics** — cross-field invariants the JSON Schema cannot express are
   ...
   by an ``uncertainty.per_output`` block**: a model with an uncovered output is
...
(``.errors``) and the source (``.source``) so a consumer (a model registry, for
example) can surface them. The non-raising :func:`collect_manifest_errors`
returns the same list for callers that prefer to aggregate.

Out of scope here (deliberately): discovering packages on disk, reloading them,
and checking that ``invocation.executable`` exists on disk -- those belong to
whatever dispatches the model. Keeping the executable check out of the contract
layer is what lets the worked example — which ships no ``.exe`` — stay
contract-valid. Patterns (e.g. the ``id`` regex) live only in the JSON Schema to
avoid a schema/code divergence; the typed layer mirrors required fields and
enums, which downstream code switches on.
"""
...
#
# Shipping the schema as package data means there is no frozen-bundle special
# case (no `sys._MEIPASS` lookup): the schema travels inside the wheel, so there
# is one resolution path and it is the same in a source tree, a wheel and a
# frozen exe.
...
# here fails CI (a schema/code divergence).
# --------------------------------------------------------------------------- #
...
# consumer can flag them rather than silently mis-render.
DEFERRED_VIEWERS: frozenset[Viewer] = frozenset({Viewer.field_contour})


# Manifest-spec (schema) versioning + back-compat policy. `spec_version` is
# MAJOR.MINOR; a manifest omitting it is assumed to target the v1 spec
# (DEFAULT_SPEC_VERSION). A consumer loads any manifest whose MAJOR is
# <= SUPPORTED_SPEC_MAJOR — minor bumps are additive and back-compatible, and
# unknown additive fields are ignored by the typed model — and rejects a higher
# MAJOR loud. This is the version-independence that lets one consumer build load
# models authored against a later same-major spec without a rebuild; a breaking
# (major) spec is refused honestly, never mis-rendered.
...
class Manifest(BaseModel):
    ...
    # cannot know the consumer's supported major).
    spec_version: str = DEFAULT_SPEC_VERSION
    owner: Owner
    domain: str
    modality: Modality
    tags: list[str] | None = None
    # Plain-language analysis category for non-expert discovery.
...
class ManifestValidationError(Exception):
    ...
    Mirrors :class:`opencontractml.model_card.ModelCardValidationError` in shape so
    a caller can catch ``(ManifestValidationError, ModelCardValidationError)``
...
def _spec_version_errors(manifest: Manifest) -> list[str]:
    """Reject a manifest authored against a newer-MAJOR spec than this module supports.

    The JSON Schema validates `spec_version`'s *shape* (``^\\d+\\.\\d+$``) but
    cannot enforce *compatibility* — it does not know this module's
    :data:`SUPPORTED_SPEC_MAJOR`. That back-compat policy lives here. A missing ``spec_version`` has already defaulted to
    ...
    if major > SUPPORTED_SPEC_MAJOR:
        return [
            f"spec_version {raw!r} targets manifest-spec major {major}, but this "
            f"validator supports major <= {SUPPORTED_SPEC_MAJOR}; upgrade open-contract-ml "
            "to load this model (back-compat policy)"
        ]
    return []
...
def semantic_errors(manifest: Manifest) -> list[str]:
    ...
    # Manifest-spec compatibility gate: a newer-major spec than this module
    # supports is refused before its (possibly reinterpreted)
...
def manifest_warnings(manifest: Manifest) -> list[str]:
    """Advisory, non-rejecting checks for non-expert discoverability.

    These never block a model from loading — they are warnings, not errors.
    They nudge authors toward the lay-readable, analysis-first metadata a
    non-CAE/ML user with an analysis in mind needs to find the model. Surfaced
    by the pre-publish gate CLI (``python -m opencontractml.manifest``).
    """
    warnings: list[str] = []
...
def load_model(manifest_path: Path) -> ValidatedModel:
    ...
    It does **not** check that ``invocation.executable`` exists — that
    filesystem check belongs to whatever dispatches the model, and leaving it
    out keeps the worked example (no ``.exe`` on disk) contract-valid.
    """
...
# --------------------------------------------------------------------------- #
# Pre-publish gate. Aggregate a *package*'s contract errors without raising, so
# an author can fix a candidate model before publishing it. Reuses the very
# validators load_model uses, so the gate can never diverge from load-time
# validation.
# --------------------------------------------------------------------------- #
def validate_package(path: Path) -> list[str]:
    ...
    :func:`collect_manifest_errors`, and the mandatory-section + id/version
    ...
    that is the dispatcher's check at load time, and skipping it lets a
    contract-only worked example (which ships no ``.exe``) pass the gate. A real
    package's executable is verified when the model is loaded for dispatch.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    ``python -m opencontractml.manifest <package-dir-or-manifest> [...]``. This is
    the command an author runs against a candidate package before publishing it.
    """
    parser = argparse.ArgumentParser(
        prog="python -m opencontractml.manifest",
