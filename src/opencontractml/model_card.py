"""Model-card validation: front-matter + the mandatory H2 sections.

The model card is the mandatory human-prose companion to the manifest. This
module validates:

* **Front-matter** — YAML between leading ``---`` fences declaring ``model_id``
  and ``version``. When the manifest's id/version are supplied
  (:func:`opencontractml.manifest.load_model` always supplies them), the card's must match — this catches a stale card
  pasted next to a newer manifest.
* **Structure** — the mandatory H2 sections, in order, as the card's *leading*
  H2 headings. ``## References`` and any other extra H2s are permitted, but
  only *after* the mandatory set. The set is the Contract's reconciled section
  list (``docs/spec/CONTRACT-v1.md`` section 4.2), taken from
  :data:`opencontractml.verify.CARD_SECTIONS` rather than retyped, so this
  validator and ``open-contract-ml check`` (rule ``C003``) cannot disagree
  about which sections a card needs.
...
:class:`opencontractml.manifest.ManifestValidationError`. The non-raising
:func:`collect_model_card_errors` returns the same list. This module imports
nothing from :mod:`opencontractml.manifest` — the dependency runs one way (manifest ->
...
from .verify import CARD_SECTIONS
...
# The mandatory H2 sections, in the exact required order: the Contract's eleven.
# This used to be a separate nine-section list, so the same card could pass one
# validator in this package and fail the other; it is now the checker's own
# tuple, and a test asserts the two are the same object.
MANDATORY_SECTIONS: tuple[str, ...] = CARD_SECTIONS
    ...
    for index, expected in enumerate(MANDATORY_SECTIONS):
        ...
        if actual != expected:
            errors.append(
                ...
                f"(the {required} mandatory sections must come first, in order)"
            )
    return errors
