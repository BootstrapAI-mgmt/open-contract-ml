...
``model_gate.py`` used to call ``pickle.load`` on ``model_bundle.pkl`` directly.
Pickle is not a data format -- it is a little stack language, and its ``REDUCE``
opcode calls whatever callable the byte stream names.  So "validate the
customer's model bundle" was a remote code execution primitive: a bundle whose
...
D9 says: serving runs on numpy/ONNX, sklearn estimators go to ``skops``, and an
