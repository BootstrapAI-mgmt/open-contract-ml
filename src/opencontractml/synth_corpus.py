"""Physically grounded synthetic PV-C2 corpus in the exact PSW04 schema (rung 1.5).

Response source: thermal-mesh-calculators' ``SingleLayerShieldCalculator``
(Newton-Raphson shield energy balance, deterministic -- verticals plan P8),
...
Swept parameters are emitted as dotted paths in the vocabulary a
cfd-automation zone-thermal overlay would use (verticals plan TC-PSW07):
``zones.exhaust.t_surf_K``, ``zones.underhood.t_air_K``,
``zones.underhood.h_out_W_m2K``, ``zones.heat_shield.eps_out``.
``mesh_size_mm`` / ``n_cells`` are derived from the same tmc boundary-driven
sizing cfd-automation's Stage 3 uses, so the mesh columns are realistic
provenance rather than filler.

Defect modes generate the G1 fail demonstrations (verticals plan section 4.2):
  --defect mesh-only     the F-1 shape: the swept axis is ``mesh.t_fluid_K``,
...
try:
    ...
    raise SystemExit("thermal-mesh-calculators is required: pip install -e <path to the tmc clone> (%s)" % exc)
...
def build(n: int, seed: int, defect: str, data_card: bool, hash_column: bool, out: Path) -> Path:
    ...
    for i, pt in enumerate(points, start=1):
        case_id = "case_%04d" % i
        if defect == "mesh-only":
            phys_point = dict(NOMINAL)                     # physics never changes: the F-1 shape
    ...
    if data_card:
        write_json(out / "data_card.json", {
            "corpus_file": written.name, "schema": "PSW04 (cfd-automation TC-PSW04)",
            "generator": "synth_corpus.py rung-1.5 (thermal-mesh-calculators %s shield solver)" % TMC_VERSION,
            ...
            "date_range_utc": [started, utc_now()], "analyst": "SESSION-10B synth_corpus.py",
