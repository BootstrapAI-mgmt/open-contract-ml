"""M1 -- corpus-native B1 trainer for PSW04-schema corpora (per component, per target).

Implements section 4.2 M1 of VERTICALS-BUILD-TEST-VALIDATE-PLAN.md:

  M1.1 split      random test split + a CORNER split by parameter region (TASK-5 B1
                  Stage 4): rows at or above the corner quantile of the declared
...
Usage:
  python train_b1.py --corpus <dir>/training_corpus.parquet --gate-report out/corpus_gate_report.json
                     --rules rules/pv-c2.physics_rules.json --out out/model [--component heat_shield]

The gate report is REQUIRED: the trainer consumes the G1 admissible-row index,
so nothing can be trained on a corpus the gate did not admit (ladder rule i).
"""
...
def fit_models(X_fit: np.ndarray, y_fit: np.ndarray, X_cal: np.ndarray, y_cal: np.ndarray,
    ...
    # anisotropic-RBF Gaussian process (TASK-5 B1 Stage 6 names GP / kriging; the TASK-8
    # thermal cell deferred its GP variant -- this is it)
