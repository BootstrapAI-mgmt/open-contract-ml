...
thirteen files carried citation anchors into an internal quote ledger that is
deliberately never published, so a reader of the public repository had no way to
follow the reference. The anchors were replaced by ``docs/REFERENCES.md``. This
file is the gate that stops the class of defect coming back, in whatever form --
a link to a doc that was withdrawn, a path that only ever existed upstream, a
schema directory that was not part of the extraction.
...
#: Links that were already dangling before this gate existed, with the target
#: each one points at. Every one of them is a path this repository does not ship:
#: ``schemas/`` and ``templates/`` were not part of the extraction, the workflow
#: file and ``ROADMAP.md`` live upstream, and ``docs/gui-design.md`` was never
#: written. Fixing them means either shipping the target or rewriting the prose,
#: and both are edits to documents outside the citation change -- so they are
#: recorded here rather than silently tolerated or hastily patched.
KNOWN_DANGLING = {
    ("docs/model-onboarding.md", "docs/gui-design.md"),
    ("docs/model-onboarding.md", "schemas/manifest.schema.json"),
    ("docs/model-onboarding.md", "templates/model_card.template.md"),
    ("docs/model-onboarding.md", ".github/workflows/validate-model-package.yml"),
    ("docs/model-onboarding.md", "ROADMAP.md"),
    ("docs/model-packages.md", "schemas/manifest.schema.json"),
    ("docs/model-packages.md", "templates/model_card.template.md"),
}
...
#: hypothetical: the session that retired the namespace recorded losing five of
#: thirteen files to exactly that mistake.
RETIRED_ANCHOR = re.compile(rf"\b{_RETIRED_PREFIX}(?:-[A-Za-z0-9]+)*-[0-9]+[a-z]?\b")

#: The two files allowed to name a retired anchor, for the same reason
#: ``test_provenance.SHORTHAND_EXEMPT`` exists: they are the *record of* the
#: thing rather than a use of it. ``PROVENANCE.yaml`` must state accurately why
#: a source file was deferred, and that reason quotes the label the upstream
#: file gives itself; ``test_provenance.py`` tabulates the anchors of the
#: passages it detects. Everything else in the tree — including reports about
#: the removal — writes retired keys without the prefix.
ANCHOR_EXEMPT = {"PROVENANCE.yaml", "tests/test_provenance.py"}


def test_no_retired_citation_anchor_survives_anywhere(tracked):
    """No file may carry an anchor into the unpublished quote ledger.
    ...
    **This gate is deliberately not restricted to markdown.** The sweep that
    ...
    assert not hits, (
        "%d retired citation anchor(s) remain; each points into a ledger this "
