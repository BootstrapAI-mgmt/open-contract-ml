"""Tests for server.manifest — schema, typed model, and semantic validation."""
...
def test_load_model_validates_example_contract() -> None:
    ...
    # load_model must NOT require one (executable existence is TC-104's check).
...
def test_uncovered_output_rejected() -> None:
    # The keystone Wall A check: an output with no per_output block is rejected.
    data = _valid()
...
# --------------------------------------------------------------------------- #
# Schema parity — the typed layer must mirror the schema exactly (Wall A). A
# schema edit that is not reflected in the enums/required fields fails here.
# --------------------------------------------------------------------------- #
...
def test_spec_version_is_optional_not_in_required_set() -> None:
    # Acceptance 4: spec_version is additive + optional — it must not appear in
...
# --------------------------------------------------------------------------- #
# Manifest-spec versioning + back-compat policy (TC-GUI17, design §13 dec. 15)
# --------------------------------------------------------------------------- #
def test_spec_version_absent_defaults_to_v1() -> None:
    # Acceptance 1: a manifest omitting spec_version validates and the typed
...
def test_brake_disc_example_omits_spec_version_and_still_loads() -> None:
    # Acceptance 1 against the real worked example, which predates the field.
...
def test_higher_major_spec_version_rejected_loud() -> None:
    # Acceptance 2: a newer-major spec is rejected, not silently dropped.
    data = _valid()
...
# --------------------------------------------------------------------------- #
# Pre-publish package gate (TC-GUI18) — validate_package aggregates manifest +
# card errors without raising, reusing the registry's own validators.
# --------------------------------------------------------------------------- #
...
def test_validate_package_accepts_brake_disc_example() -> None:
    # Acceptance 3: the worked example is a conformant package — and the gate
...
def test_validate_package_rejects_nonconformant_citing_failure() -> None:
    # Acceptance 2: an uncovered-output package is rejected, naming `z`.
...
# --------------------------------------------------------------------------- #
# Non-expert discoverability: analysis_type + advisory warnings (TC-GUI15)
# --------------------------------------------------------------------------- #
...
def test_brake_disc_example_declares_analysis_type() -> None:
    # Acceptance 3: the worked example declares analysis_type and still validates.
...
def test_manifest_warnings_flag_missing_lay_readable_prose() -> None:
    # Acceptance 4: missing analysis_type / when_to_use surface advisory warnings.
...
def test_manifest_warnings_are_advisory_not_errors() -> None:
    # Acceptance 4: the same omissions are NOT hard rejections.
    assert collect_manifest_errors(_valid()) == []
