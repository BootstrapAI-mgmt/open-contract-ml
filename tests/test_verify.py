...
This is LESSONS L18 applied to the checker itself. The thing this repo's scalar
ladder got wrong -- V2.3 conservation "passes" when a free-text field is a
non-empty string, and a test enshrined the pass -- is the failure mode these
tests exist to prevent in the contract.
"""
...
def base_manifest() -> dict:
    return {
        ...
        "provenance": {
            ...
            "code": {"repo": "BootstrapAI-mgmt/CAE-ML-data-pipelines", "commit": "0123456789abcdef"},
...
def test_every_error_rule_has_a_negative_test():
    """A rule with no demonstrated failure is an unproven rule (LESSONS L18)."""
...
# --------------------------------------------------------------------------- #
# The two rules the ecosystem actually got wrong. Worth their own tests.
# --------------------------------------------------------------------------- #
def test_a_declaration_string_is_not_a_conservation_measurement(tmp_path: Path):
    ...
    `examples/TASK-10-corpus-consumer/model_gate.py` grades V2.3 by
...
# --------------------------------------------------------------------------- #
# The gap measurement must stay honest when a producer is absent.
# --------------------------------------------------------------------------- #
def test_gap_reports_source_missing_rather_than_inventing(tmp_path: Path):
    repo = Path(__file__).resolve().parents[1]
    src = vs.Sources(repo, tmp_path / "no_gui", tmp_path / "no_ps")
    rows = vs.build_gap_rows(src)
    assert rows, "gap matrix must not be empty"
    verdicts = {r["ps_report"][0] for r in rows} | {r["gui"][0] for r in rows}
    assert vs.NOTFOUND in verdicts, "absent producers must read SOURCE-MISSING, not ABSENT"
    assert src.notes, "a missing source must be recorded in the probe notes"


# --------------------------------------------------------------------------- #
# The shipped schema artifacts must not drift from the checker (cae-ml-gui calls
# this guard Wall A: a schema and the code that mirrors it diverge silently).
# --------------------------------------------------------------------------- #
...
def test_checker_emits_lf_only(tmp_path: Path):
    ...
    checkout. The reference package tripped exactly this during G1D.
    """
    pkg = write_package(tmp_path / "pkg")
    out_json = tmp_path / "report.json"
    vocab = tmp_path / "vocab.json"
    matrix = tmp_path / "gap.md"
    assert vs.main(["check", str(pkg), "--json", str(out_json)]) == 0
    assert vs.main(["vocabulary", "--out", str(vocab)]) == 0
    assert vs.main(["gap", "--out", str(matrix), "--gui-root", str(tmp_path / "none"),
                    "--physsur-root", str(tmp_path / "none")]) == 0
    for path in (out_json, vocab, matrix):
